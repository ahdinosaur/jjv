...
//           copy = Object.create(Object.getPrototypeOf(obj));
  ...
  var resolveURI = function (env, schema_stack, uri) {
    ...
    if (hash_idx === -1) {
      if (!env.descriptors.hasOwnProperty(uri))
        return null;
      return [env.descriptors[uri], [env.descriptors[uri].schema]];
    }
    ...
    if (hash_idx > 0) {
      ...
      if (!env.schema.hasOwnProperty(name)) {
        ...
        env = [env.descriptors[name]];
        schema_stack = [env.descriptors[name].schema];
    } else {
      if (!schema_stack)
        return null;
      uri = uri.substr(1);
  ...
  var checkValidity = function (env, schema_stack, object_stack, options) {
    ...
    if (schema.hasOwnProperty('$ref')) {
      var env_and_schema_stack = resolveURI(env, schema_stack, schema.$ref);
      env = env_and_schema_stack[0];
      schema_stack = env_and_schema_stack[1];
      if (!schema_stack)
    ...
    for (v in schema) {
      if (schema.hasOwnProperty(v) && !handled.hasOwnProperty(v)) {
        if (v === 'format') {
          ...
          if (env.fieldValidate.hasOwnProperty(v) && !env.fieldValidate[v](prop, schema[v].hasOwnProperty('$data') ? resolveObjectRef(object_stack, schema[v].$data)[1] : schema[v], schema, object_stack, options)) {
            objerrs[v] = true;
            malformed = true;
          }
        }
  ...
  function Descriptor(env, schema, options) {
    ...
    this.defaultOptions = clone(options);
  }
  ...
  Environment.prototype = {
    validate: function (name, object, options) {
      var env, schema_stack = [name], errors = null, object_stack = [{object: {'__root__': object}, key: '__root__'}];

      if (typeof name === 'string') {
        var env_and_schema_stack = resolveURI(this, null, name);
        env = env_and_schema_stack[0];
        schema_stack = env_and_schema_stack[1];
        if (!schema_stack)
      ...
      errors = checkValidity(env, schema_stack, object_stack, options);
    ...
    addDescriptor: function (name, descriptor) {
      ...
      if (descriptor.hasOwnProperty('id') && typeof descriptor.id === 'string' && descriptor.id !== name) {
        if (descriptor.id.charAt(0) === '/')
          throw new Error('jjv: descriptor id\'s starting with / are invalid.');
        this.descriptors[normalizeID(descriptor.id)] = descriptor;
      } else if (!name) {
        throw new Error('jjv: descriptor needs either a name or id attribute.');
      }
      if (name)
        this.descriptors[normalizeID(name)] = descriptor;
    },

    addSchema: function (name, schema) {
      var descriptor = new Descriptor(this, schema, this.defaultOptions);
